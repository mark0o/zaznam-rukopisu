...
input[type="radio"]:checked {
  outline: 8px solid var(--main-colour);
}
...
function trimMinimalWhitespace(text) {
  ...
  return lines
    .map((line) => line.replace(new RegExp(`^\\s{${minIndent}}`), ""))
    .join("\n");
}
...
function cycleCodeBlocks() {
  ...
  codeBlocks.forEach((codeBlock) => {
    ...
    codeBlockElement.children[0].textContent = `${trimBlankLines(
      trimMinimalWhitespace(codeText)
    )}`;
  });
}
...
if (
  document.querySelector("body").hasAttribute("mdp-show-loading-screen") ===
  true
) {
...
function setMouse() {
  if (
    document.querySelector("body").hasAttribute("mdp-custom-cursor") === true
  ) {
...
document.querySelector("head").innerHTML =
  document.querySelector("head").innerHTML + icons;
